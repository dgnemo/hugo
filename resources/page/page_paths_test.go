	...
	for _, langPrefixPath := range []string{"", "no"} {
		for _, langPrefixLink := range []string{"", "no"} {
			for _, uglyURLs := range []bool{false, true} {
				for _, omitHTMLExtensionFromURLs := range []bool{false, true} {
					...
					for i, test := range tests {
						t.Run(fmt.Sprintf("langPrefixPath=%s,langPrefixLink=%s,uglyURLs=%t,omitHTMLExtensionFromURLs=%t,name=%s", langPrefixPath, langPrefixLink, uglyURLs, omitHTMLExtensionFromURLs, test.name),
							func(t *testing.T) {
								...
								test.d.PathSpec.OmitHTMLExtensionFromURLs = omitHTMLExtensionFromURLs
								...
								if omitHTMLExtensionFromURLs {
									expected.Link = strings.TrimSuffix(expected.Link, ".html")
								}

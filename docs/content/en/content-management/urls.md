...
If your webserver is configured to serve `filename.html` when the URL `/filename` is requested (and perform a 301 redirect when to `/filename` when `/filename.html` is requested) you can set `omitHTMLExtensionFromURLs = true` or `omitHTMLExtensionFromURLs: true` in your site's `config.toml` or `config.yaml`, respectively.
In this way all the Permalink generated by hugo will omit the `.html` extension and point to `/filename` directly.
